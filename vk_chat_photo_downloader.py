...
def get_photos_urls(vk, user_id, directory):
    ...
    start_from = 0
    while True:
        results = vk.messages.getHistoryAttachments(peer_id=user_id, media_type="photo",
                                                    count=200, start_from=start_from)
        if results['items']:
            for attachment in results['items']:
                photo = attachment['attachment']['photo']
                photo_name = f"{photo['id']}_{photo['owner_id']} " \
            ...
            hues.log(f"Получено {len(photos_urls)} фото, следующее смещение: {start_from}")
        else:
...
def get_peer_id(vk):
    while True:
        peer_id = input("Введите ID (пользователя/беседы/сообщества) для начала загрузки фото: ")
        try:
            peer_id = int(peer_id)
        except ValueError:
        ...
        try:
            ...
            hues.info(f"Найден пользователь {results[0]['first_name']} {results[0]['last_name']}")
            break
...
def main():
    ...
    hues.success(f"--- {len(output)} фотографий скачано за {end_time - start_time} секунд ---")
